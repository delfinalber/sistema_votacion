...
async function cargarTabResultados() {
    ...
    try {
        const res = await fetch('Votaciones/obtener_resultados.php');
        ...
        const bloque = (titulo, lista) => `
            <div class="resultado-cargo-admin">
                ...
                ${lista.map(i => `
                    <div class="resultado-item-admin">
                        <span>${i.nombre}</span>
                        <span class="votos-count">${i.total_votos}</span>
                    </div>
                `).join('')}
            </div>
        `;
        ...
        cont.innerHTML = `
            ${bloque('🎓 Resultados Personero Estudiantil', data.personero)}
            ${bloque('⚖️ Resultados Contralor Estudiantil', data.contralor)}
            <div class="resultado-cargo-admin">
                <h4>📊 Resumen General</h4>
                <div class="resultado-item-admin"><span>Total de votantes registrados</span><span class="votos-count">${data.resumen.total_registrados}</span></div>
                <div class="resultado-item-admin"><span>Total que votaron</span><span class="votos-count">${data.resumen.total_votaron}</span></div>
                <div class="resultado-item-admin"><span>Participación</span><span class="votos-count">${data.resumen.participacion}%</span></div>
            </div>
        `;
    } catch {
